...
//    include_once('../connect_db.php');
    ...
    include_once('includes/fetch_culture.php');
    include_once('includes/fetch_design.php');
    include_once('includes/fetch_politics.php');
    include_once('includes/fetch_social.php');
    include_once('includes/fetch_sports.php');
$article=new Article;
$articles =$article->fetch_all();
$article_tech=new Article_tech;
$articles_tech =$article_tech->fetch_all();
$article_culture=new Article_culture;
$articles_culture =$article_culture->fetch_all();
$article_design=new Article_design;
$articles_design =$article_design->fetch_all();
$article_politics=new Article_politics;
$articles_politics =$article_politics->fetch_all();
$article_social=new Article_social;
$articles_social =$article_social->fetch_all();
$article_sports=new Article_sports;
$articles_sports =$article_sports->fetch_all();
session_start();
...
$connection = mysqli_connect($servername, $username, $password) or die("Error " . mysqli_error());
    ...
    $db = mysqli_select_db($connection, "gse") or die("Error " . mysqli_error());

   $sql = mysqli_query($connection, "SELECT * FROM articles");
...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
	...
	<script src="ArticlesDataset.js"></script>
	<script>
		const category_normalization_factor = 100;
		const views_normalization_factor = 100;	
		
		let trainingData = [
			{ input: {category: category_culture, hasComments: false,views:10 }, output: { wanted: 1 }},
            { input: {category: category_culture, hasComments: false,views:20 }, output: { wanted: 0.8}},
            { input: {category: category_culture, hasComments: false,views:150 }, output: { wanted: 0.5 }},
            { input: {category: category_culture, hasComments: true,views:30 }, output: { wanted: 0.3 }},
            { input: {category: category_culture, hasComments: false,views:25 }, output: { wanted: 0.2 }},
            { input: {category: category_culture, hasComments: true,views:150 }, output: { wanted: 0.1 }},
            { input: {category: category_culture, hasComments: false,views:100 }, output: { wanted: 0.9 }},
            { input: {category: category_culture, hasComments: false,views:45 }, output: { wanted: 0.75 }},
            { input: {category: category_culture, hasComments: true,views:10 }, output: { wanted: 0.3 }},
            { input: {category: category_culture, hasComments: false,views:88 }, output: { wanted: 0.1 }}
		];
		let ratingItem;
...
//			document.getElementById('result').innerHTML = '';
			...
			let suggestionItems = [];
			for(i=0;i<ArticlesDataset.length;i++){
            ...
            let reco_array = [];
            for(i=0;i<6;i++){
                reco_array[i]=suggestionItems[i].displayingInformation.article_id;
            }
            for(i=0;i<6;i++){
                console.log(reco_array[i]);
            }
            
             window.location.href = "index.php?reco_arr=" + reco_array[0]+"&reco_arr1="+reco_array[1]+"&reco_arr2="+reco_array[2]+"&reco_arr3="+reco_array[3]+"&reco_arr4="+reco_array[4]+"&reco_arr5="+reco_array[5];
//            $.post("/homepage.php", {reco1:reco_array[0],reco2:reco_array[1],reco3:reco_array[2],reco4:reco_array[3],reco5:reco_array[4],reco6:reco_array[5]}); 
            //var a=JSON.stringify(reco_array);
//            dataString = ??? ; // array?
//                var jsonString = JSON.stringify(reco_array);
            
//            var obj={};
//            obj.reco1=reco_array[0];
//            obj.reco2=reco_array[1];
//            obj.reco3=reco_array[2];
//            obj.reco4=reco_array[3];
//            obj.reco5=reco_array[4];
//            obj.reco6=reco_array[5];
//            
//            
//            
//                $.ajax({
...
//                var params = url.split('?reco=');
//                var reco = (params[6]);
//                    $.ajax({
//                    type:"POST",
//                    url:"homepage.php",
//                    data:{reco:reco_array},
//                    success:function(){
//                    }
//                    });
//                });
...
//			ratingItem = getNormalizedItemFromStock(Math.floor((Math.random() * ArticlesDataset.length - 1) + 1));			
//			document.getElementById('result').innerHTML += 
...
//				'</div>';
		...
		function getNormalizedItemFromStock(index){
			const item = ArticlesDataset[index];
			trainingInformation = item.trainingInformation;
			return {
				trainingInformation : {
					category: trainingInformation.category / category_normalization_factor,
					hasComments: trainingInformation.hasComments,
					views: trainingInformation.views / views_normalization_factor
				},
...
//				'category: ' + getCategoryName(trainingInformation.category) + '<br/>' +
//				'has comments?: ' + (1 === trainingInformation.hasComments ? 'yes' : 'no') + '<br/>' +
//				'Views: ' + ((Math.round((trainingInformation.views * views_normalization_factor) * 100)) / 100) ;
//			return formattedItem;
//		}
        function getFormattedItem(item){
			const trainingInformation = item.trainingInformation;
			const formattedItem = 
                '<p>'+item.displayingInformation.article_id+'<br>'+
				'category: ' + getCategoryName(trainingInformation.category) + '<br/>' +
				'has comments?: ' + (1 === trainingInformation.hasComments ? 'yes' : 'no') + '<br/>' +
				'Views: ' + ((Math.round((trainingInformation.views * views_normalization_factor) * 100)) / 100) ;
			return formattedItem;
		...
		function getCategoryName(category){
			var id = category * category_normalization_factor;
			var name = 
				category_culture === id ? 'culture' :
				category_design === id ? 'design' :
				category_politics === id ? 'politics' :
				category_social === id ? 'social' :
                category_tech === id ? 'tech' :
                category_sports === id ? 'sports' :
				'';
			return name;			
		}
        nextTry();
    ...
    <div class="container">
        ...
        <div class="row justify-content-center align-items-center brandrow">
            ...
            <div class="col-sm-2 col-md-2">
                                     ...
                                     <a class="menuzord-brand" href="index.php"><img  src="https://www.gsered.com/images/logo.png" alt="" href="/index.php"></a>
            </div>
...
//    echo $array_size;
                ...
                {
                    if(($art['count'] == $array_count[0]) && ($art['article_id'] == $array_id[0]))
                    {
                        break;
                    }
 ...
 ?>
<!--    <a href="https://www.gsered.com/blog_system/gse_pdo/blogshow.php?id=42" class="pull-right flip text-gray font-13"><i class="fa fa-angle-double-right text-theme-color-2"></i> Read more</a>-->
    
    
        <div class="row listrecent">
                    <a href="blogshow.php?id=<?php echo $art['article_id']; ?>">    <div class="col-md-4 col-lg-4 col-sm-4 padr10" id="post-101">
                    <div class="card post highlighted">
            
            <img src=" admin/uploads/<?php echo $art['article_img']; ?>" >            <div class="card-block">
...
<!--
                <span class="author-meta"> 
                    ...
                    echo date('l jS',$art['article_timestamp']);
                ?></span>
                    <span class="dot"></span>
...
//                   echo " ";
                ...
                for($j=0;$j<$array_size;$j++)
                {
                    if(($art['count'] == $array_count[1]) && ($art['article_id'] == $array_id[1]))
                    {
//                        $array_id[0] = 0;
                        break;
                    }
...
<a href="blogshow.php?id=<?php echo $art['article_id']; ?>"><img src=" admin/uploads/<?php echo $art['article_img']; ?>" ></a> <div class="card-block">
    <h2 class="card-title">
        <a href="blogshow.php?id=<?php echo $art['article_id']; ?>"><p><?php if(strlen($art['article_title'])<34) echo $art['article_title']; else echo substr($art['article_title'],0,32)."..."; ?></p></a>
    </h2>
            <div class="metafooter">                        
            <div class="wrapfooter"> 
<!--
...
//                   $art=next($articles);                           
                for($j=0;$j<$array_size;$j++)
                {
                    ...
                    if(($art['count'] == $array_count[2]) && ($art['article_id'] == $array_id[2]))
                    {
//                        
                        break;
                    }
...
//                   echo " ";
                ...
                for($j=0;$j<$array_size;$j++)
                {
                    if(($art['count'] == $array_count[3]) && ($art['article_id'] == $array_id[3]))
                    {
//                        $array_id[0] = 0;
                        break;
                    }
...
//                   echo " ";
                ...
                for($j=0;$j<$array_size;$j++)
                {
                    if(($art['count'] == $array_count[4]) && ($art['article_id'] == $array_id[4]))
                    {
//                        $array_id[0] = 0;
                        break;
                    }
...
//    echo $reco_arr[3];
    ...
    ?>
        ...
        <h2> 
            ...
            <a class="d-block pull-right morefromcategory" href="tech.php">
            ...
            <div class="row">
                ...
                <?php foreach($articles_tech as $article_tech){ ?>
                    <div class="col-xs-12 col-sm-10 col-md-10">
                        <article class="post media-post clearfix pb-0 mb-10" id="article_box">
                            <div class="row">
                      
                                <div class="col-sm-8">
                                    <a class="post-thumb" id="name" href="blogshow.php?id=<?php echo $article_tech['article_id']; ?>">
                                    </a>
                                    <!--     <img src="https://gsered.com/blog_system/gse_pdo/admin/uploads/<?/*php echo $article['article_img'];*/?>" width="20" height="20" alt="">
                                    -->
                                    <div class="post-right">
                                        <h5 class="entry-title text-uppercase mt-0 mb-5"><a href="blogshow.php?id=<?php echo $article_tech['article_id']; ?>"><?php echo $article_tech['article_title']; ?></a></h5>
                                    
                                        <p class="post-date mb-10 font-12">
                                        By Admin,Posted
                                        <?php  
                                        echo date('l jS',$article_tech['article_timestamp']);
                                        ?>
                                        </p>
                                        <p><?php echo substr($article_tech['article_content'],0,300) ?>...</p>
                      
                                        <a href="blogshow.php?id=<?php echo $article_tech['article_id']; ?>" class="pull-right flip text-gray font-13"><i class="fa fa-angle-double-right text-t#heme-color-2"></i> Read more</a>  
                                    </div>
                                </div>
                      
                                <div class="col-sm-4">
                                    <img src=" admin/uploads/<?php echo $article_tech['article_img'];?>" class="img-responsive" width="150px" height="300px"
    ...
    <!--<section class="featured-posts">        
            <div class="row listfeaturedtag margneg10">
                    ...
                    <div class="card" id="post-111">
    <div class="row">
                <div class="col-md-5 wrapthumbnail">
            <a href="https://www.themepush.com/demo-mediumish/marketers-get-in-touch-with-your-audience/">
                <div class="thumbnail" style="background-image:url(https://www.themepush.com/demo-mediumish/wp-content/uploads/sites/29/2017/11/fig-08-11-2017_00-43-21.jpg);">
                </div>
            </a>        
        </div>
                <div class=" col-md-7 ">
            <div class="card-block">
                <h2 class="card-title"><a href="https://www.themepush.com/demo-mediumish/marketers-get-in-touch-with-your-audience/">Marketers: Get in touch with your audience</a></h2>
                <span class="card-text d-block">Promotional products are more popular than ever, based on data from a 2016 consumer survey conducted by Promotional Products Association...</span>
                <div class="metafooter">
                    <div class="wrapfooter"> 
                        <span class="meta-footer-thumb"> 
                            <a href="https://www.themepush.com/demo-mediumish/author/themesforwebcom/">
                        ...
                        <span class="post-read-more">
                            <a href="https://www.themepush.com/demo-mediumish/marketers-get-in-touch-with-your-audience/" title="">
                            <svg class="svgIcon-use" width="25" height="25" viewBox="0 0 25 25">
                                ...
                                <div class="col-md-6 col-lg-6 col-sm-6 padlr10">
                    <div class="card" id="post-41">
    <div class="row">
                <div class="col-md-5 wrapthumbnail">
            <a href="https://www.themepush.com/demo-mediumish/food-fuel-and-finance-the-3fs-of-economics/">
                <div class="thumbnail" style="background-image:url(https://www.themepush.com/demo-mediumish/wp-content/uploads/sites/29/2017/11/fig-08-11-2017_15-16-28.jpg);">
                </div>
            </a>        
        </div>
                <div class=" col-md-7 ">
            <div class="card-block">
                <h2 class="card-title"><a href="https://www.themepush.com/demo-mediumish/food-fuel-and-finance-the-3fs-of-economics/">Food, Fuel, and Finance … The 3Fs of Economics</a></h2>
                <span class="card-text d-block">Nearly half of U.S. high school students say they do not know how to establish good credit and more...</span>
                <div class="metafooter">
                    <div class="wrapfooter"> 
                        <span class="meta-footer-thumb"> 
                            <a href="https://www.themepush.com/demo-mediumish/author/sununderthesky/">
                        ...
                        <span class="post-read-more">
                            <a href="https://www.themepush.com/demo-mediumish/food-fuel-and-finance-the-3fs-of-economics/" title="">
                            <svg class="svgIcon-use" width="25" height="25" viewBox="0 0 25 25">
                                <path d="M19 6c0-1.1-.9-2-2-2H8c-1.1 0-2 .9-2 2v14.66h.012c.01.103.045.204.12.285a.5.5 0 0 0 .706.03L12.5 16.85l5.662 4.126a.508.508 0 0 0 .708-.03.5.5 0 0 0 .118-.285H19V6zm-6.838 9.97L7 19.636V6c0-.55.45-1 1-1h9c.55 0 1 .45 1 1v13.637l-5.162-3.668a.49.49 0 0 0-.676 0z" fill-rule="evenodd"></path>
                            </svg>
                            </a>
                                ...
                                <div class="col-md-6 col-lg-6 col-sm-6 padlr10">
                    <div class="card" id="post-28">
    <div class="row">
                <div class="col-md-5 wrapthumbnail">
            <a href="https://www.themepush.com/demo-mediumish/how-to-increase-your-business-exposure-with-seo/">
                <div class="thumbnail" style="background-image:url(https://www.themepush.com/demo-mediumish/wp-content/uploads/sites/29/2017/11/fig-07-11-2017_19-59-06.jpg);">
                </div>
            </a>        
        </div>
                <div class=" col-md-7 ">
            <div class="card-block">
                <h2 class="card-title"><a href="https://www.themepush.com/demo-mediumish/how-to-increase-your-business-exposure-with-seo/">How to Increase your Business Exposure with SEO</a></h2>
                <span class="card-text d-block">These instances were not known to the man of whom we write, but lessstriking ones of the same character...</span>
                <div class="metafooter">
                    <div class="wrapfooter"> 
                        <span class="meta-footer-thumb"> 
                            <a href="https://www.themepush.com/demo-mediumish/author/themesforwebcom/">
                        ...
                        <span class="post-read-more">
                            <a href="https://www.themepush.com/demo-mediumish/how-to-increase-your-business-exposure-with-seo/" title="">
                            <svg class="svgIcon-use" width="25" height="25" viewBox="0 0 25 25">
                                <path d="M19 6c0-1.1-.9-2-2-2H8c-1.1 0-2 .9-2 2v14.66h.012c.01.103.045.204.12.285a.5.5 0 0 0 .706.03L12.5 16.85l5.662 4.126a.508.508 0 0 0 .708-.03.5.5 0 0 0 .118-.285H19V6zm-6.838 9.97L7 19.636V6c0-.55.45-1 1-1h9c.55 0 1 .45 1 1v13.637l-5.162-3.668a.49.49 0 0 0-.676 0z" fill-rule="evenodd"></path>
                            </svg>
                            </a>
                                ...
                                <div class="col-md-6 col-lg-6 col-sm-6 padlr10">
                    <div class="card" id="post-10">
    <div class="row">
                <div class="col-md-5 wrapthumbnail">
            <a href="https://www.themepush.com/demo-mediumish/the-review-as-a-form-of-welcome/">
                <div class="thumbnail" style="background-image:url(https://www.themepush.com/demo-mediumish/wp-content/uploads/sites/29/2017/11/fig-07-11-2017_22-43-36.jpg);">
                </div>
            </a>        
        </div>
                <div class=" col-md-7 ">
            <div class="card-block">
                <h2 class="card-title"><a href="https://www.themepush.com/demo-mediumish/the-review-as-a-form-of-welcome/">Headache-Free Moving Advice: 3 Steps Closer to a New&hellip;</a></h2>
                <span class="card-text d-block">One summer night a man stood on a low hill overlooking a wide expanse of forest and field. By...</span>
                <div class="metafooter">
                    <div class="wrapfooter"> 
                        <span class="meta-footer-thumb"> 
                            <a href="https://www.themepush.com/demo-mediumish/author/nickname/">
                        ...
                        <span class="post-read-more">
                            <a href="https://www.themepush.com/demo-mediumish/the-review-as-a-form-of-welcome/" title="">
                            <svg class="svgIcon-use" width="25" height="25" viewBox="0 0 25 25">
                                <path d="M19 6c0-1.1-.9-2-2-2H8c-1.1 0-2 .9-2 2v14.66h.012c.01.103.045.204.12.285a.5.5 0 0 0 .706.03L12.5 16.85l5.662 4.126a.508.508 0 0 0 .708-.03.5.5 0 0 0 .118-.285H19V6zm-6.838 9.97L7 19.636V6c0-.55.45-1 1-1h9c.55 0 1 .45 1 1v13.637l-5.162-3.668a.49.49 0 0 0-.676 0z" fill-rule="evenodd"></path>
                            </svg>
                            </a>
        ...
        <h2> 
            <span>Culture &nbsp;</span>
            <a class="d-block pull-right morefromcategory" href="culture.php">
                More &nbsp; <i class="fa fa-angle-right"></i> 
            </a>
            <div class="clearfix"></div> 
        </h2>
    ...
    <div class="row">
                ...
                <?php foreach($articles_culture as $article_culture){ ?>
              <div class="col-xs-12 col-sm-10 col-md-10">
                <article class="post media-post clearfix pb-0 mb-10">
                  <div class="row">
                      
    <div class="col-sm-8">
                    <a class="post-thumb" id="name" href="blogshow.php?id=<?php echo $article_culture['article_id']; ?>">
                      </a>
        <!--     <img src="https://gsered.com/blog_system/gse_pdo/admin/uploads/<?/*php echo $article['article_img'];*/?>" width="20" height="20" alt="">
        -->
                  <div class="post-right">
                    <h5 class="entry-title text-uppercase mt-0 mb-5"><a href="blogshow.php?id=<?php echo $article_culture['article_id']; ?>"><?php echo $article_culture['article_title']; ?></a></h5>
                                    
    <p class="post-date mb-10 font-12">
        By Admin,Posted
                <?php  
                    echo date('l jS',$article_culture['article_timestamp']);
                ?>
                </p>
                    <p><?php echo substr($article_culture['article_content'],0,300) ?>...</p>
                      
                    <a href="blogshow.php?id=<?php echo $article_culture['article_id']; ?>" class="pull-right flip text-gray font-13"><i class="fa fa-angle-double-right text-t#heme-color-2"></i> Read more</a>  
                  </div>
                      </div>
                      
    <div class="col-sm-4">
        <img src=" admin/uploads/<?php echo $article_culture['article_img'];?>" class="img-responsive" width="150px" height="300px"
             onerror="this.onerror=null;this.src='images/pattern3.png';" alt="images/favicon.png"> 
    ...
    <!--<section class="featured-posts">        
            <div class="row listfeaturedtag margneg10">
                                <div class="col-md-6 col-lg-6 col-sm-6 padlr10">
                    <div class="card" id="post-128">
    <div class="row">
                <div class="col-md-5 wrapthumbnail">
            <a href="https://www.themepush.com/demo-mediumish/encourage-a-love-of-reading-during-national-family-literacy-month/">
                <div class="thumbnail" style="background-image:url(https://www.themepush.com/demo-mediumish/wp-content/uploads/sites/29/2017/11/fig-08-11-2017_01-21-19.jpg);">
                </div>
            </a>        
        </div>
                <div class=" col-md-7 ">
            <div class="card-block">
                <h2 class="card-title"><a href="https://www.themepush.com/demo-mediumish/encourage-a-love-of-reading-during-national-family-literacy-month/">Encourage a Love of Reading During National Family Literacy&hellip;</a></h2>
                <span class="card-text d-block">For many parents, it&#8217;s a struggle to get kids to read outside of the classroom, but November is the...</span>
                <div class="metafooter">
                    <div class="wrapfooter"> 
                        <span class="meta-footer-thumb"> 
                            <a href="https://www.themepush.com/demo-mediumish/author/nickname/">
                        ...
                        <span class="post-read-more">
                            <a href="https://www.themepush.com/demo-mediumish/encourage-a-love-of-reading-during-national-family-literacy-month/" title="">
                            <svg class="svgIcon-use" width="25" height="25" viewBox="0 0 25 25">
                                <path d="M19 6c0-1.1-.9-2-2-2H8c-1.1 0-2 .9-2 2v14.66h.012c.01.103.045.204.12.285a.5.5 0 0 0 .706.03L12.5 16.85l5.662 4.126a.508.508 0 0 0 .708-.03.5.5 0 0 0 .118-.285H19V6zm-6.838 9.97L7 19.636V6c0-.55.45-1 1-1h9c.55 0 1 .45 1 1v13.637l-5.162-3.668a.49.49 0 0 0-.676 0z" fill-rule="evenodd"></path>
                            </svg>
                            </a>
                                ...
                                <div class="col-md-6 col-lg-6 col-sm-6 padlr10">
                    <div class="card" id="post-125">
    <div class="row">
                <div class="col-md-5 wrapthumbnail">
            <a href="https://www.themepush.com/demo-mediumish/tips-for-keeping-kids-occupied-on-road-trips/">
                <div class="thumbnail" style="background-image:url(https://www.themepush.com/demo-mediumish/wp-content/uploads/sites/29/2017/11/fig-08-11-2017_01-16-00.jpg);">
                </div>
            </a>        
        </div>
                <div class=" col-md-7 ">
            <div class="card-block">
                <h2 class="card-title"><a href="https://www.themepush.com/demo-mediumish/tips-for-keeping-kids-occupied-on-road-trips/">Tips for Keeping Kids Occupied on Road Trips</a></h2>
                <span class="card-text d-block">On the road, drivers must deal with constant traffic concerns, including glaring sunlight, blinding rain, traffic jams, street signs...</span>
                <div class="metafooter">
                    <div class="wrapfooter"> 
                        <span class="meta-footer-thumb"> 
                            <a href="https://www.themepush.com/demo-mediumish/author/themesforwebcom/">
                        ...
                        <span class="post-read-more">
                            <a href="https://www.themepush.com/demo-mediumish/tips-for-keeping-kids-occupied-on-road-trips/" title="">
                            <svg class="svgIcon-use" width="25" height="25" viewBox="0 0 25 25">
                                <path d="M19 6c0-1.1-.9-2-2-2H8c-1.1 0-2 .9-2 2v14.66h.012c.01.103.045.204.12.285a.5.5 0 0 0 .706.03L12.5 16.85l5.662 4.126a.508.508 0 0 0 .708-.03.5.5 0 0 0 .118-.285H19V6zm-6.838 9.97L7 19.636V6c0-.55.45-1 1-1h9c.55 0 1 .45 1 1v13.637l-5.162-3.668a.49.49 0 0 0-.676 0z" fill-rule="evenodd"></path>
                            </svg>
                            </a>
                                ...
                                <div class="col-md-6 col-lg-6 col-sm-6 padlr10">
                    <div class="card" id="post-121">
    <div class="row">
                <div class="col-md-5 wrapthumbnail">
            <a href="https://www.themepush.com/demo-mediumish/five-tips-for-aspiring-travel-writers/">
                <div class="thumbnail" style="background-image:url(https://www.themepush.com/demo-mediumish/wp-content/uploads/sites/29/2017/11/fig-08-11-2017_01-13-19.jpg);">
                </div>
            </a>        
        </div>
                <div class=" col-md-7 ">
            <div class="card-block">
                <h2 class="card-title"><a href="https://www.themepush.com/demo-mediumish/five-tips-for-aspiring-travel-writers/">Five Tips for Aspiring Travel Writers</a></h2>
                <span class="card-text d-block">As the media landscape keeps changing around us, travel writers are following many paths to success. Some are book authors, some...</span>
                <div class="metafooter">
                    <div class="wrapfooter"> 
                        <span class="meta-footer-thumb"> 
                            <a href="https://www.themepush.com/demo-mediumish/author/sununderthesky/">
                        ...
                        <span class="post-read-more">
                            <a href="https://www.themepush.com/demo-mediumish/five-tips-for-aspiring-travel-writers/" title="">
                            <svg class="svgIcon-use" width="25" height="25" viewBox="0 0 25 25">
                                <path d="M19 6c0-1.1-.9-2-2-2H8c-1.1 0-2 .9-2 2v14.66h.012c.01.103.045.204.12.285a.5.5 0 0 0 .706.03L12.5 16.85l5.662 4.126a.508.508 0 0 0 .708-.03.5.5 0 0 0 .118-.285H19V6zm-6.838 9.97L7 19.636V6c0-.55.45-1 1-1h9c.55 0 1 .45 1 1v13.637l-5.162-3.668a.49.49 0 0 0-.676 0z" fill-rule="evenodd"></path>
                            </svg>
                            </a>
                                ...
                                <div class="col-md-6 col-lg-6 col-sm-6 padlr10">
                    <div class="card" id="post-118">
    <div class="row">
                <div class="col-md-5 wrapthumbnail">
            <a href="https://www.themepush.com/demo-mediumish/honoring-moms-how-3-inspiring-women-are-changing-the-world/">
                <div class="thumbnail" style="background-image:url(https://www.themepush.com/demo-mediumish/wp-content/uploads/sites/29/2017/11/fig-08-11-2017_01-10-16.jpg);">
                </div>
            </a>        
        </div>
                <div class=" col-md-7 ">
            <div class="card-block">
                <h2 class="card-title"><a href="https://www.themepush.com/demo-mediumish/honoring-moms-how-3-inspiring-women-are-changing-the-world/">Honoring Moms : How 3 Inspiring Women are Changing&hellip;</a></h2>
                <span class="card-text d-block">No doubt about it, mothers are the backbone of the family. Theirs is a difficult balance of wife, mother,...</span>
                <div class="metafooter">
                    <div class="wrapfooter"> 
                        <span class="meta-footer-thumb"> 
                            <a href="https://www.themepush.com/demo-mediumish/author/nickname/">
                        ...
                        <span class="post-read-more">
                            <a href="https://www.themepush.com/demo-mediumish/honoring-moms-how-3-inspiring-women-are-changing-the-world/" title="">
                            <svg class="svgIcon-use" width="25" height="25" viewBox="0 0 25 25">
                                <path d="M19 6c0-1.1-.9-2-2-2H8c-1.1 0-2 .9-2 2v14.66h.012c.01.103.045.204.12.285a.5.5 0 0 0 .706.03L12.5 16.85l5.662 4.126a.508.508 0 0 0 .708-.03.5.5 0 0 0 .118-.285H19V6zm-6.838 9.97L7 19.636V6c0-.55.45-1 1-1h9c.55 0 1 .45 1 1v13.637l-5.162-3.668a.49.49 0 0 0-.676 0z" fill-rule="evenodd"></path>
                            </svg>
                            </a>
        ...
        <div class="section-title">
            <h2>
                <span>Design &nbsp;</span>
                <a class="d-block pull-right morefromcategory" href="design.php">
                    More &nbsp; <i class="fa fa-angle-right"></i> 
                </a>
                <div class="clearfix"></div> 
            </h2>
    ...
    <div class="row">
                ...
                <?php foreach($articles_design as $article_design){ ?>
              <div class="col-xs-12 col-sm-10 col-md-10">
                <article class="post media-post clearfix pb-0 mb-10">
                  <div class="row">
                      
    <div class="col-sm-8">
                    <a class="post-thumb" id="name" href="blogshow.php?id=<?php echo $article_design['article_id']; ?>">
                      </a>
        <!--     <img src="https://gsered.com/blog_system/gse_pdo/admin/uploads/<?/*php echo $article['article_img'];*/?>" width="20" height="20" alt="">
        -->
                  <div class="post-right">
                    <h5 class="entry-title text-uppercase mt-0 mb-5"><a href="blogshow.php?id=<?php echo $article_design['article_id']; ?>"><?php echo $article_design['article_title']; ?></a></h5>
                                    
    <p class="post-date mb-10 font-12">
        By Admin,Posted
                <?php  
                    echo date('l jS',$article_design['article_timestamp']);
                ?>
                </p>
                    <p><?php echo substr($article_design['article_content'],0,300) ?>...</p>
                      
                    <a href="blogshow.php?id=<?php echo $article_design['article_id']; ?>" class="pull-right flip text-gray font-13"><i class="fa fa-angle-double-right text-t#heme-color-2"></i> Read more</a>  
                  </div>
                      </div>
                      
    <div class="col-sm-4">
        <img src=" admin/uploads/<?php echo $article_design['article_img'];?>" class="img-responsive" width="150px" height="300px"
             onerror="this.onerror=null;this.src='images/pattern3.png';" alt="images/favicon.png"> 
...
<div class="section-title">
    ...
    <div class="row">
                ...
                <?php foreach($articles_politics as $article_politics){ ?>
              <div class="col-xs-12 col-sm-10 col-md-10">
                <article class="post media-post clearfix pb-0 mb-10">
                  <div class="row">
                      
    <div class="col-sm-8">
                    <a class="post-thumb" id="name" href="blogshow.php?id=<?php echo $article_politics['article_id']; ?>">
                      </a>
        <!--     <img src="https://gsered.com/blog_system/gse_pdo/admin/uploads/<?/*php echo $article['article_img'];*/?>" width="20" height="20" alt="">
        -->
                  <div class="post-right">
                    <h5 class="entry-title text-uppercase mt-0 mb-5"><a href="blogshow.php?id=<?php echo $article_politics['article_id']; ?>"><?php echo $article_politics['article_title']; ?></a></h5>
                                    
    <p class="post-date mb-10 font-12">
        By Admin,Posted
                <?php  
                    echo date('l jS',$article_politics['article_timestamp']);
                ?>
                </p>
                    <p><?php echo substr($article_politics['article_content'],0,300) ?>...</p>
                      
                    <a href="blogshow.php?id=<?php echo $article_politics['article_id']; ?>" class="pull-right flip text-gray font-13"><i class="fa fa-angle-double-right text-t#heme-color-2"></i> Read more</a>  
                  </div>
                      </div>
                      
    <div class="col-sm-4">
        <img src=" admin/uploads/<?php echo $article_politics['article_img'];?>" class="img-responsive" width="150px" height="300px"
             onerror="this.onerror=null;this.src='images/pattern3.png';" alt="images/favicon.png"> 
...
<div class="section-title">
            <h2>
                <span>Social &nbsp;</span>
                <a class="d-block pull-right morefromcategory" href="social.php">
                    More &nbsp; <i class="fa fa-angle-right"></i> 
                </a>
                <div class="clearfix"></div> 
            </h2>
    ...
    <div class="row">
                ...
                <?php foreach($articles_social as $article_social){ ?>
              <div class="col-xs-12 col-sm-10 col-md-10">
                <article class="post media-post clearfix pb-0 mb-10">
                  <div class="row">
                      
    <div class="col-sm-8">
                    <a class="post-thumb" id="name" href="blogshow.php?id=<?php echo $article_social['article_id']; ?>">
                      </a>
        <!--     <img src="https://gsered.com/blog_system/gse_pdo/admin/uploads/<?/*php echo $article['article_img'];*/?>" width="20" height="20" alt="">
        -->
                  <div class="post-right">
                    <h5 class="entry-title text-uppercase mt-0 mb-5"><a href="blogshow.php?id=<?php echo $article_social['article_id']; ?>"><?php echo $article_social['article_title']; ?></a></h5>
                                    
    <p class="post-date mb-10 font-12">
        By Admin,Posted
                <?php  
                    echo date('l jS',$article_social['article_timestamp']);
                ?>
                </p>
                    <p><?php echo substr($article_social['article_content'],0,300) ?>...</p>
                      
                    <a href="blogshow.php?id=<?php echo $article_social['article_id']; ?>" class="pull-right flip text-gray font-13"><i class="fa fa-angle-double-right text-t#heme-color-2"></i> Read more</a>  
                  </div>
                      </div>
                      
    <div class="col-sm-4">
        <img src=" admin/uploads/<?php echo $article_social['article_img'];?>" class="img-responsive" width="150px" height="300px"
             onerror="this.onerror=null;this.src='images/pattern3.png';" alt="images/favicon.png"> 
...
<div class="section-title">
            <h2>
                <span>Sports &nbsp;</span>
                <a class="d-block pull-right morefromcategory" href="sports.php">
                    More &nbsp; <i class="fa fa-angle-right"></i> 
                </a>
                <div class="clearfix"></div> 
            </h2>
    ...
    <div class="row">
                ...
                <?php foreach($articles_sports as $article_sports){ ?>
              <div class="col-xs-12 col-sm-10 col-md-10">
                <article class="post media-post clearfix pb-0 mb-10">
                  <div class="row">
                      
    <div class="col-sm-8">
                    <a class="post-thumb" id="name" href="blogshow.php?id=<?php echo $article_sports['article_id']; ?>">
                      </a>
        <!--     <img src="https://gsered.com/blog_system/gse_pdo/admin/uploads/<?/*php echo $article['article_img'];*/?>" width="20" height="20" alt="">
        -->
                  <div class="post-right">
                    <h5 class="entry-title text-uppercase mt-0 mb-5"><a href="blogshow.php?id=<?php echo $article_sports['article_id']; ?>"><?php echo $article_sports['article_title']; ?></a></h5>
                                    
    <p class="post-date mb-10 font-12">
        By Admin,Posted
                <?php  
                    echo date('l jS',$article_sports['article_timestamp']);
                ?>
                </p>
                    <p><?php echo substr($article_sports['article_content'],0,300) ?>...</p>
                      
                    <a href="blogshow.php?id=<?php echo $article_sports['article_id']; ?>" class="pull-right flip text-gray font-13"><i class="fa fa-angle-double-right text-t#heme-color-2"></i> Read more</a>  
                  </div>
                      </div>
                      
    <div class="col-sm-4">
        <img src=" admin/uploads/<?php echo $article_sports['article_img'];?>" class="img-responsive" width="150px" height="300px"
             onerror="this.onerror=null;this.src='images/pattern3.png';" alt="images/favicon.png"> 
...
<a href="https://www.themepush.com/demo-mediumish/category/technology/" class="tag-cloud-link tag-link-8 tag-link-position-6" style="font-size: 8pt;" aria-label="technology (4 items)">technology</a>                </div>
            </div>
        </div>-->    
